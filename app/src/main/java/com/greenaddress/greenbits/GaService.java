...
public class GaService extends Service implements INotificationHandler {
    ...
    private final Map<Integer, Map<String, String>> mCoinBalances = new HashMap<>();

    private final Map<Integer, Fiat> mFiatBalances = new HashMap<>();
    private float mFiatRate;
    private Map<Integer, Map<?,?>> mTxList = new HashMap<>();
    ...
    public ListenableFuture<Map<?, ?>> getMyTransactions(final int subAccount) {
        if (mTxList.containsKey(subAccount))
            return (ListenableFuture) Futures.immediateFuture(mTxList.get(subAccount));
    ...
    public ListenableFuture<Map<String,String>> getMapBalance(final int subAccount) {
        if (mCoinBalances.containsKey(subAccount))
            return Futures.immediateFuture(mCoinBalances.get(subAccount));
        return updateBalance(subAccount);
    }

    public ListenableFuture<Fiat> getFiatBalance(final int subAccount) {
        if (mFiatBalances.containsKey(subAccount))
            return Futures.immediateFuture(mFiatBalances.get(subAccount));
